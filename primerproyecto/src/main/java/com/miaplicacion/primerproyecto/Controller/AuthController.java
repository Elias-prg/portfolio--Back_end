...
public class AuthController {
    ...
    @PostMapping("/login")
        public String login(@RequestBody LoginRequest loginRequest) {
               ...
               return encoder.encode(JwtEncoderParameters.from(createJwtClaims(authentication))).getTokenValue();
               // return jwtEncoder.encode(JwtEncoderParameters.from(createJwtClaims(authentication))).getTokenValue();
             }
        
        private JwtClaimsSet createJwtClaims(Authentication authentication) {
        Instant now = Instant.now();
        long expiry = 36000L;
        String scope = authentication.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)
